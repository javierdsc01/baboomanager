...
export const POST = async (req) => {
    try {
        ...
        // Obtenemos competicionId del body
        const body = await req.json();
        const { competicionId } = body;

        // Definir el payload para la solicitud
        const payload = new URLSearchParams({
            option: 2,
            id_competition: competicionId,
        }).toString();
        ...
        const headers = {
            ...
            'Cookie': `refresh-token=${token}; token=${token};`,
        };

        // Realizar la solicitud POST al endpoint
        const response = await axios.post('https://mister.mundodeportivo.com/ajax/onboarding.php', payload, { headers });
