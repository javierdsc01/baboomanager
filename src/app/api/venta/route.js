...
export const POST = async (req) => {
    try {
        ...
        const { id_player, action, price } = body;

        // Validar que todos los parámetros están presentes
        if (!id_player || !action || !price) {
        ...
        const payload = new URLSearchParams({
            id_player: id_player,
            action: action,
            price: price.toString()  // Asegurarse de que el price sea una cadena
        }).toString();
        ...
        const response = await axios.post('https://mister.mundodeportivo.com/ajax/sale', payload, { headers });
